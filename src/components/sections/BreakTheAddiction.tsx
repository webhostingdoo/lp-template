...
const amenityIcons = [
  <svg key="0" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="5" r="1.5"/><path d="M7 22l2.5-7 2.5 3 2.5-3L17 22"/><path d="M8 12l1.5-4h5L16 12"/></svg>,
  ...
  <svg key="2" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a9 9 0 0 1 6 2.5c-2 1-4 1.5-6 1.5s-4-.5-6-1.5A9 9 0 0 1 12 2z"/><path d="M12 6c0 4-3 7-7 9"/><path d="M12 6c0 4 3 7 7 9"/><path d="M8 21h8"/><path d="M12 15v6"/></svg>,
  <svg key="3" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M23 12a11.05 11.05 0 0 0-22 0zm-5 7a3 3 0 0 1-6 0v-7"/></svg>,
...
export default function BreakTheAddiction(props: BreakTheAddictionProps) {
  const {
    eyebrow = config.breakTheAddiction.eyebrow,
    h2 = config.breakTheAddiction.heading,
    body = config.breakTheAddiction.body,
  } = props;
    ...
    >
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <ul className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-16">
          {config.breakTheAddiction.amenities.map((label, i) => (
            <li key={label} className="flex items-center gap-3 text-white text-lg">
              <span className="text-accent shrink-0">{amenityIcons[i % amenityIcons.length]}</span>
              {label}
            </li>
          ))}
        </ul>
      ...
      <div className="quote-section">
        ...
        <div className="relative max-w-3xl mx-auto px-4 md:px-8 text-center">
          <blockquote className="quote-text">
            &ldquo;{config.breakTheAddiction.quote}&rdquo;
          </blockquote>
          <div>
            <div className="quote-name">{config.breakTheAddiction.quoteAuthor}</div>
            <div className="quote-title">{config.breakTheAddiction.quoteTitle}</div>
          </div>
        </div>
      </div>
    </section>
